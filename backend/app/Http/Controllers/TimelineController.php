...
class TimelineController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        if ($request->boolean('favorite')) {
            $query->whereHas('favorites', fn ($q) => $q->where('users.id', $request->user()->id));
        }
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        foreach ($request->file('photos', []) as $file) {
            if (! str_starts_with((string) $file->getMimeType(), 'image/')) {
                throw ValidationException::withMessages([
                    'photos' => ['Timeline attachments must be images.'],
                ]);
            }
        }

        $event = DB::transaction(function () use ($request, $space, $validated, $storageDisk) {
            ...
            foreach ($request->file('photos', []) as $file) {
                $path = $file->store("spaces/{$space->id}/timeline/{$event->id}", $storageDisk);
                TimelineFile::create([
            ...
            if (filter_var($request->input('isFavorite', false), FILTER_VALIDATE_BOOLEAN)) {
                $event->favorites()->attach($request->user()->id);
            }
        ...
        Zawsze::notifyPartner(
            ...
            $request->user(),
            'timeline.created',
            $request->user()->name.' added “'.$event->title.'” to your timeline.',
            ['timelineId' => $event->id],
        );
    ...
    public function update(Request $request, TimelineEvent $timeline): JsonResponse
    {
        ...
        if (array_key_exists('title', $validated)) {
            $timeline->title = trim($validated['title']);
        }
        if (array_key_exists('eventDate', $validated)) {
            $timeline->event_date = $validated['eventDate'] ? Carbon::parse($validated['eventDate']) : null;
        }
        if (array_key_exists('eventType', $validated)) {
            $timeline->event_type = $validated['eventType'];
        }
        if (array_key_exists('description', $validated)) {
            $timeline->description = $validated['description'];
        }
        if (array_key_exists('isLocked', $validated)) {
            $timeline->is_locked = $validated['isLocked'];
        }
        $timeline->save();

        if (array_key_exists('isFavorite', $validated)) {
            if ($validated['isFavorite']) {
                $timeline->favorites()->syncWithoutDetaching([$request->user()->id]);
            } else {
                $timeline->favorites()->detach($request->user()->id);
            }
        }
    ...
    public function media(TimelineFile $file, MediaResponseService $media): Response
    {
        return $media->inline(
            $file->storage_disk,
            $file->storage_path,
    ...
    public function payload(Request $request, TimelineEvent $event): array
    {
        ...
        return [
            ...
            'files' => $locked ? [] : $event->files->map(fn (TimelineFile $file) => [
                'id' => $file->id,
                'url' => Zawsze::signed('api.timeline.media', ['file' => $file->id]),
            ])->values(),
